...
def help_():
    doc = '''
Just an Ordinary and Easy-to-use Windows file manager

Navigate files fast using partial searches with no additional text

Press `Enter` to go into a directory, open a file, or use a command

Pressing `Enter` on a partial search will select the first option in the suggestions

Pressing `Tab` will autofill the partial search with the first option in the suggestions
...
runcmd - run custom command in new terminal
...
Example:
`::runcmd >> cd path/to/preferred/directory && <command>`
...
def run_script_in_new_terminal(command):
    ...
    with open(tmppath, 'a') as f:
        pass
    try:
        powershell_command = f'Start-Process powershell -ArgumentList "-NoExit", "-Command", "{command} | Tee-Object -FilePath {tmppath}"'
    ...
    if flag == 2:
        if com == 'rename' or com == 'moveto':
            ...
            try:
                ...
                print(f"Couldn't copy file: {err}")
        elif com == 'editor':
            try:
                subprocess.run(f"start {arg} {fullpath}", shell=True)
            except subprocess.CalledProcessError as e:
...
def get_var(currpath, pathlist, input_chars, cmd_chars, tokens, history, history_index, varbuf = [], invarlist = ['$']):
    ...
    clear_current_line()
    size = os.get_terminal_size()
    disp = os.path.join(currpath, tokens) if tokens else currpath
    disp_curr = disp.split(os.path.sep)
    ...
    try:
        ...
        while True:
            if msvcrt.kbhit():
                ...
                elif char == '\xe0':  # Special key prefix
...
def main():
    os.system('cls')
    print("JOEMAMA 1.3 (Windows Version)")
