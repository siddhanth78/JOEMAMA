...
Just an Ordinary and Easy-to-use Mac OS file manager (For Windows)\r
...
Example:\r
`::runcmd >> cd path\\to\\preferred\\directory && <bash command>`\r
    ...
    try:
        ...
        while True:
            if msvcrt.kbhit():
                ...
                if char.startswith(b'\xe0') == False:
                ...
                if char == '\n' or char == '\r':
                    ...
                    if query == '--help':
                        ...
                        if os.path.isfile(currpath):
                            try:
                                subprocess.run(f"start {currpath}", shell=True, text=True)
                            except subprocess.CalledProcessError as e:
